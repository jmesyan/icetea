...
var protobuf = load("res://net/pomelo_protobuf.gd").new() 
var protocol = load("res://net/pomelo_protocol.gd").new()
...
var packageOffset = 0
var packageSize = 0
var packageBuffer = PoolByteArray()
var state = ST_HEAD
var headBuffer = PoolByteArray()
...
func _getBodySize():
	...
	for i in [1,2,3]:
		...
		leng += headBuffer[i]#headBuffer.readUInt8(i)
	return leng

func _readHead(datas,offset):
	var hlen = headSize - headOffset
	var dlen = datas.size() - offset
	var leng = min(hlen,dlen)
	var dend = offset + leng
	#data.copy(headBuffer,headOffset,offset,dend)
	for i in range(leng):
		headBuffer.set(headOffset+i, datas[i])
...
func _onopen():
	var buf = protocol.strencode(to_json(handshakeBuffer))
...
func _sendMessage(reqId,route,msg):
	...
	if clientProtos!=null and clientProtos.has(route):
		...
		msg = protocol.strencode(to_json(msg))
...
func _initData(data):
	...
	if protos != null :
		...
		if protobuf != null:
			...
			protobuf.init(d)
		localStorage.set_value("pomelo","protos",to_json(protos))

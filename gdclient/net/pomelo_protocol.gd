...
class Message:
	...
	func encode(id,type,compressRoute,route,msg):
		...
		buffer = res[1]
		if _parent._msgHasId(type):
			res = _parent._encodeMsgId(id,buffer,offset)
			offset = res[0]
			buffer = res[1]
		if _parent._msgHasRoute(type):
			res = _parent._encodeMsgRoute(compressRoute,route,buffer,offset)
			offset = res[0]
			buffer = res[1]
		if msg != null:
			# encode msg body
			for i in range(msg.size()):
				buffer.set(offset+i, msg[i])
			offset += msg.size()
		return buffer
...
func strdecode_old(buffer):
	var bytes = buffer#PoolByteArray()

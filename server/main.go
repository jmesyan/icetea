...
func (mgr *RoomManager) Join(s *session.Session, msg []byte) error {
    ...
    s.Push("onMembers", &protos.AllMembers{Members: room.group.Members()})
    // notify others
    room.group.Broadcast("onNewUser", &protos.NewUser{Content: fmt.Sprintf("New user: %d", s.ID())})
    // new user join group
    room.group.Add(s) // add session to group
    return s.Response(&protos.JoinResponse{Result: "success"})
}

// Message sync last message to all members
func (mgr *RoomManager) Message(s *session.Session, msg *protos.UserMessage) error {
    if !s.HasKey(roomIDKey) {
